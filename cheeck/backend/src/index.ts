...
const readDB = () => {
  const defaultDb: Database = { manga: [] };
...
app.post("/api/manga", (req, res) => {
  ...
  const existingManga = db.manga.find((m: StoredManga) => m.mal_id === newMangaData.mal_id);
  if (existingManga) {
  ...
  const newManga: StoredManga = { 
    ...newMangaData, 
    id: Date.now(),
    rating: newMangaData.rating || 0,
    notes: newMangaData.notes || "",
    tags: newMangaData.tags || [],
    genres: newMangaData.genres || [],
...
app.post("/api/manga/import", (req, res) => {
  const db = readDB();
  const importedManga: StoredManga[] = req.body;
  ...
  importedManga.forEach(incoming => {
    const existingIndex = db.manga.findIndex((m: StoredManga) => m.mal_id === incoming.mal_id);
    if (existingIndex >= 0) {
      // Overwrite existing with imported, but keep the current generated ID to avoid breaking UI keys
      db.manga[existingIndex] = { ...incoming, id: db.manga[existingIndex].id };
      updatedCount++;
    } else {
      db.manga.unshift({ ...incoming, id: Date.now() + Math.random() });
      addedCount++;
    }
  });
...
app.put("/api/manga/:id", (req, res) => {
  ...
  const mangaIndex = db.manga.findIndex((m: StoredManga) => m.id === mangaId);
  ...
  let readingHistory = existingManga.reading_history || [];
  const oldChaptersRead = existingManga.chapters_read || 0;
  const newChaptersRead = req.body.chapters_read !== undefined ? req.body.chapters_read : oldChaptersRead;
  
  if (newChaptersRead > oldChaptersRead) {
    const chaptersDiff = newChaptersRead - oldChaptersRead;
    const now = new Date();
    const today = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')}`;
    const todayEntryIndex = readingHistory.findIndex((entry: any) => entry.date === today);
    
    if (todayEntryIndex >= 0) {
      readingHistory[todayEntryIndex].count += chaptersDiff;
    } else {
      readingHistory.push({ date: today, count: chaptersDiff });
    }
  }

  const chaptersRead = newChaptersRead;
  ...
  const updatedManga = { ...existingManga, ...req.body, reading_history: readingHistory };
  ...
  updatedManga.last_updated = Date.now();
...
app.delete("/api/manga/:id", (req, res) => {
  ...
  db.manga = db.manga.filter((m: StoredManga) => m.id !== mangaId);
...
app.get("/api/stats", (req, res) => {
  const db = readDB();
  const mangaList = db.manga || [];
  
  const totalManga = mangaList.length;
  const completedManga = mangaList.filter((m: StoredManga) => m.status === "Completed").length;
  const totalChaptersRead = mangaList.reduce((sum: number, m: StoredManga) => sum + (m.chapters_read || 0), 0);
  
  // Aggregate reading history across all manga
  const historyMap: Record<string, number> = {};
  mangaList.forEach((m: StoredManga) => {
    if (m.reading_history) {
      m.reading_history.forEach((entry) => {
        historyMap[entry.date] = (historyMap[entry.date] || 0) + entry.count;
      });
    }
  });

  const readingHistory = Object.keys(historyMap).map(date => ({
    date,
